            ...
            <div className="w-full h-full max-w-7xl mx-auto p-4 md:p-8 flex flex-col">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 rounded-full bg-theme-primary flex items-center justify-center text-white font-bold text-lg shadow-lg">
                            {call.name ? call.name.substring(0, 2).toUpperCase() : user?.username?.substring(0, 2).toUpperCase()}
                        </div>
                        <div>
                            <h3 className="text-theme font-bold text-xl">
                                {callAccepted && !callEnded ? call.name || "User" : call.name || "Connecting..."}
                            </h3>
                            <p className="text-theme-secondary text-sm flex items-center space-x-2">
                ...
                {/* Video Container */}
                <div className="flex-1 relative rounded-3xl overflow-hidden bg-theme-surface border border-theme-border shadow-2xl">
                        ...
                        {callAccepted && !callEnded ? (
                                ...
                                {callType === 'video' ? (
                                    ...
                                    <>
                                        ...
                                        <div className="flex flex-col items-center justify-center space-y-6">
                                            <div className="w-40 h-40 rounded-full bg-theme-primary flex items-center justify-center text-white font-bold text-6xl shadow-2xl animate-pulse">
                                                {call.name ? call.name.substring(0, 2).toUpperCase() : "U"}
                                            </div>
                                            <div className="text-center">
                                                <h4 className="text-theme font-bold text-2xl mb-2">{call.name || "User"}</h4>
                                                <p className="text-theme-secondary flex items-center justify-center space-x-2">
                                                    <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                                                    <span>Connected</span>
                                                </p>
                                            </div>
                            ...
                            <div className="flex flex-col items-center justify-center space-y-6">
                                <div className="relative">
                                    ...
                                    <div className="absolute inset-0 rounded-full border-4 border-theme-primary animate-ping opacity-20"></div>
                                </div>
                                <div className="text-center">
                                    <h4 className="text-theme font-bold text-2xl mb-2">{call.name || "Calling..."}</h4>
                                    <p className="text-theme-secondary flex items-center justify-center space-x-2">
                                        <span className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse"></span>
                                        <span>Ringing...</span>
                                    </p>
                    ...
                    {callType === 'video' && stream && (
                        <div
                            ...
                            className="absolute bottom-6 right-6 w-48 h-36 rounded-2xl overflow-hidden bg-theme-surface border-2 border-theme-primary shadow-2xl hover:shadow-theme-primary/20 hover:border-theme-primary transition-shadow z-20"
                        >
                            {stream ? (
                                <video
                                    playsInline
                                ...
                                <div className="w-full h-full flex items-center justify-center bg-theme-surface">
                                    <div className="w-16 h-16 rounded-full bg-theme-primary flex items-center justify-center text-white font-bold text-2xl">
                ...
                {call.isReceivingCall && !callAccepted && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-lg z-50">
                        <div className="bg-theme-surface p-8 rounded-3xl shadow-2xl border border-theme-primary max-w-md w-full mx-4">
                            <div className="flex flex-col items-center space-y-6">
                                {/* Caller Avatar */}
                                <div className="relative">
                                    <div className="w-32 h-32 rounded-full bg-theme-primary flex items-center justify-center text-white font-bold text-5xl shadow-2xl">
                                        {call.name ? call.name.substring(0, 2).toUpperCase() : "U"}
                                    </div>
                                    <div className="absolute inset-0 rounded-full border-4 border-theme-primary animate-ping opacity-20"></div>
                                </div>
                                ...
                                {/* Caller Info */}
                                <div className="text-center">
                                    <h3 className="text-2xl text-theme font-bold mb-2">{call.name}</h3>
                                    <p className="text-theme-secondary mb-1">Incoming {call.callType === 'audio' ? 'Voice' : 'Video'} Call</p>
                                    <div className="flex items-center justify-center space-x-2 text-sm text-theme-secondary">
                ...
                <div className="flex items-center justify-center gap-4 mt-6">
                    <button
                        ...
                        className={`group relative p-5 rounded-full transition-all duration-200 shadow-xl ${micOn
                            ? 'bg-theme-surface hover:bg-theme-surface-hover border border-theme-border text-theme'
                            : 'bg-red-500 hover:bg-red-600 border border-red-400/50 text-white'
